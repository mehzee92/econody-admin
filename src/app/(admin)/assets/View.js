...
export default function TokenizeAssetForm({onClose, asset}) {
  ...
  const {  fetchAssets } = useAssetsStore();
    ...
    const updateAsset = async(e) => {
        e.preventDefault();
        const result = await postData("/api/assets/update", formData);
    ...
    const deleteAsset = async(e) => {
        ...
        if(!b) { return; }
        const result = await postData("/api/assets/delete", formData);
        alert(JSON.stringify(result));
        fetchAssets();
        onClose();
    }; 
            ...
            <form className="space-y-2">
               ...
               <div>
                ...
                <div className="text-right flex">
                    ...
                    <div>
                        <button onClick={updateAsset} className="font-bold bg-blue-600 text-white px-3 w-36 py-2 mx-2 rounded">Update</button>
                        <button onClick={deleteAsset} className="font-bold bg-red-600 text-white px-3 w-36 py-2 mx-2 rounded">Delete</button>
                    </div>
                </div>
            </form>
        </div>
