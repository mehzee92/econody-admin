...
import {  RWA_FACTORY_CONTRACT, OPERATIONS_CONTRACT, toWei } from '@/components/const'
import { apiUrl } from "@/components/utils";

export default function OperationCharges() {
  ...
  const fetchHolders=async(assetId)=>{
      ...
      const _holders = result.map((row)=>{
        let _amount = Math.round(1000*toEth(row.b.toString()))/1000;
        if(_data.token_standard=="ERC721") 
        {
            _amount = Math.round(toEth(row.b.toString()+"000000000000000000")); 
        }    
        supply += _amount;
        return {
  ...
  const chargeDues=async()=>
  {
      const _holders = holders.map((row)=>row.holder);
      const _amounts = holders.map((row)=>toWei((row.amount/totalSupply)*totalReward));
  ...
  return (
    ...
    <div className="max-w-sm pr-2">
      ...
      <div className="pt-2 w-80">
          <div className="py-2">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="block w-60 h-12 px-4 py-2 border border-blue-500 rounded-lg text-gray-900 placeholder:text-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-blue-300 transition-all duration-200">
                Fetch Holders
                </button>                
              </div>
          </div>
          ...
          <button
            onClick={chargeDues} type="submit" 
    ...
    <div className="px-3 flex-1">
        ...
        <div className="" style={{maxHeight:"600px", overflowY:"scroll"}}>
            ...
            <div className="grid grid-cols-3 gap-4 bg-gray-200 font-bold gap-2 py-2 border-1 border-gray-200 px-1">
                ...
                <div className="pl-2 px-2">Dues Share</div>
            </div>
            ...
            Array.isArray(holders) && holders.map((row, index)=>{
                    return (
                        <div key={"some_"+index} className="grid grid-cols-3 gap-4 gap-2 py-2 border-1 border-gray-200 px-1">
                            ...
                            <div className="pl-2 px-2">{(Math.round(10000*row.amount/totalSupply)*totalReward)/10000}</div>
                        </div>
                    )
                })
            }
